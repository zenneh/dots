{pkgs, ...}: {
  home.packages = with pkgs; [
    ...
    (pkgs.writeShellScriptBin "dwm-record" ''
      #!/usr/bin/env bash
      ...
      exec ffmpeg -f pulse -i default -ac 2 -acodec vorbis -f x11grab -s "$W"x"$H" -i :0.0+$X,$Y -c:v libx264 -pix_fmt yuv420p -vf "pad=ceil(iw/2)*2:ceil(ih/2)*2" $OUTFILE 
    '')
  ];
}
